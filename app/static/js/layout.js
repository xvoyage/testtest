...
function checkAll(chkobj) {
    if ($(chkobj).text() == lang.comm_select_all) {
        $(chkobj).children("span").text(lang.comm_cancel_all);
        $(".checkall input:enabled").prop("checked", true);
    } else {
        $(chkobj).children("span").text(lang.comm_select_all);
...
function checktxt(obj, txtId) {
    ...
    $("#" + txtId).html(lang.sms_tip.replace('{count}',txtCount).replace('{length}',smsLength));
}
//四舍五入函数
...
function ExePostBack(objId, objmsg) {
    if ($(".checkall input:checked").size() < 1) {
        $.dialog.alert(lang.dlg_tip_noselect);
        return false;
    }
    var msg = lang.dlg_tip_delete_confirm;
...
function syncthis(sobj, tobj) {
    var objs = document.form1 && document.form1[tobj];
    if (!objs && window.frames['mainframe']) {
        objs = window.frames['mainframe'].document.form1[tobj];
    }
    $(objs).each(function (i, obj) {
        obj.value = sobj.checked?1:0;
    });
}
//检查是否有选中再决定回传
function CheckPostBack(objId, objmsg) {
    var msg = lang.dlg_tip_noselect;
    if (arguments.length == 2) {
        msg = objmsg;
    }
    if ($(".checkall input:checked").size() < 1) {
...
$.fn.ruleSingleCheckbox = function () {
    var singleCheckbox = function (parentObj) {
		...
		+ '<i class="off">' + lang.comm_off + '</i>'
		+ '<i class="on">' + lang.comm_on + '</i>'
...
$.fn.ruleMultiCheckbox = function () {
    var multiCheckbox = function (parentObj) {
        ...
        parentObj.find(":checkbox,br").each(function () {
...
$.fn.ruleMultiRadio = function () {
    var multiRadio = function (parentObj) {
        ...
        parentObj.children().hide(); //隐藏内容
        var divObj = $('<div class="boxwrap"></div>').prependTo(parentObj); //前插入一个DIV
        parentObj.find('input[type="radio"],br').each(function () {
            if (this.tagName.toLowerCase() == 'br') {
                divObj.append('<br />');
            } else {
...
$.fn.ruleSingleSelect = function () {
    var singleSelect = function (parentObj) {
        ...
        var divObj = parentObj.find('.boxwrap'), titObj, itemObj, arrowObj;
        if (divObj.length < 1) {
            divObj = $('<div class="boxwrap"></div>').prependTo(parentObj); //前插入一个DIV
            //创建元素
            titObj = $('<a class="select-tit" href="javascript:;"><span></span><i></i></a>').appendTo(divObj);
            itemObj = $('<div class="select-items"><ul></ul></div>').appendTo(divObj);
            arrowObj = $('<i class="arrow"></i>').appendTo(divObj);
            //绑定单击事件
            ...
            titObj = divObj.find('.select-tit');
            itemObj = divObj.find('.select-items');
            arrowObj = divObj.find('.arrow');
            itemObj.find('ul').html('');
        }
        parentObj.children().not('.boxwrap').hide(); //隐藏内容
